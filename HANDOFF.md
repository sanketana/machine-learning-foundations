...
- **From Module 2 onward, prefer real, popular, well-documented public datasets over freshly generated ones** (see "Sourcing policy" below). Recognizable real-world data gives students transferable, authentic experience and something they can read more about online. Module 1's synthetic datasets stand; new datasets introduced from Module 2 should be sourced, not synthesized, whenever a suitable open one exists.
...
Seeing one dataset through three lenses is itself a core lesson. **The anchor dataset is the one deliberate exception to the "prefer real datasets" policy:** because a single dataset must cleanly support regression *and* classification *and* clustering, stay teen-relatable, need minimal cleaning, and carry an open license, the synthetic `student_habits.csv` (already generated in Module 1) is retained for the cross-module thread unless a real public dataset is found that satisfies all of those at once. If such a dataset is found, it must still meet the Sourcing policy below. (Decision flagged for human owner; default is keep the synthetic anchor.)

**Per-module secondary datasets (Module 2 onward — real-first):** each module introduces datasets for session examples and homework variety (e.g. housing for regression, spam-style or fraud-style for classification, shopper-segments for clustering). **Prefer a popular, well-documented public dataset for each, sourced rather than generated.** Generate synthetically only when no suitable open dataset exists, or when a specific teaching property is needed that real data doesn't provide (e.g. a precisely controlled class imbalance) — and say which in the dataset README.

**Sourcing workflow (agreed with human owner):** Claude Code cannot browse or download from Kaggle in this environment. So for each new Module 2+ dataset, Claude **proposes a specific named public dataset** (with its source/license and why it fits the session), the **human owner drops the CSV into `datasets/`**, and Claude then builds the lesson plan, notebooks, and explainer around the actual columns of that file. Do not generate session content against an assumed schema — wait for the real CSV, then read it first.

**Sourcing policy (applies to every sourced dataset):**
- **License:** open only — CC0, public domain, or an explicitly permissive license that allows redistribution. Record the **license name and the source URL** (Kaggle/UCI/data.gov/etc.) in `datasets/README.md`. If the license is unclear or non-redistributable, do not use it.
- **Vendored, never fetched:** download once and **commit the CSV into `datasets/`**. Notebooks must still run fully offline with no internet calls (§9.2) — no Kaggle API calls, no `read_csv(url)` at runtime.
- **Right-sized & relatable:** keep the in-repo file small (sample/trim to <~2,000 rows if the original is large; note the sampling in the README) and keep the topic relatable to a 13–17-year-old.
- **Cleaning matches the session:** Sessions 1–8 still need near-clean data (trim to a tidy subset of columns/rows if the raw dataset is messy); deliberate messiness is only for sessions that teach cleaning.
- **Provenance & reproducibility:** the README must state the original source, license, retrieval date, and any sampling/trimming applied, so the in-repo CSV is traceable even though it isn't regenerated by a script.
...
- All notebooks must run top-to-bottom with no errors against the **in-repo datasets** — including any sourced public datasets, which are vendored as committed CSVs (§6 Sourcing policy), never downloaded or read from a URL at runtime. Use only: python stdlib, numpy, pandas, matplotlib, scikit-learn. No seaborn, no plotly, no internet calls.
